...
export function useProjectMembers(projectId: string) {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, async (snapshot) => {
      ...
      const needsHydration = items.filter((m) => !m.displayName || isUidLike(m.displayName));
      ...
      const nameEntries = await Promise.all(
        needsHydration.map(async (m) => {
          try {
            ...
            const profileName = userSnap.exists() ? (userSnap.data()?.displayName as string | undefined) : undefined;
            return [m.uid, (profileName || "").trim()] as const;
          } catch {
            return [m.uid, ""] as const;
          }
        })
      );

      const hydratedNames = new Map(nameEntries.filter(([, name]) => !!name));
      const hydratedMembers = items.map((m) => {
        const resolvedName = hydratedNames.get(m.uid);
        if (resolvedName) return { ...m, displayName: resolvedName };
        return m;
      });
