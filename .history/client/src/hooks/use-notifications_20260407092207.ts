...
function requestPermission() {
  // In extension mode, notifications go through chrome.notifications (handled by background worker)
...
// Sync tasks to extension background worker for alarm-based notifications
function syncTasksToBackground(tasks: Task[]) {
  if (
    !isExtension ||
  ...
  try {
    const serializable = tasks
      .filter((t) => t.dueAt && !isCompletedStatus(t.status))
      .map((t) => ({ id: t.id, title: t.title, status: t.status, dueAt: t.dueAt }));
    globalThis.chrome.runtime.sendMessage({ type: "UPDATE_TASKS", tasks: serializable });
  } catch {}
}
...
export function useTaskNotifications(tasks: Task[]) {
  ...
  // Sync tasks to extension background worker whenever tasks change
  useEffect(() => {
    syncTasksToBackground(tasks);
  }, [tasks]);
  ...
  useEffect(() => {
    ...
    const checkTasks = () => {
      ...
      tasks.forEach((task) => {
        if (!task.dueAt || isCompletedStatus(task.status)) return;
