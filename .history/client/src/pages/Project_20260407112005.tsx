...
import { collection, doc, updateDoc, deleteDoc, getDoc, setDoc, getDocs, query, where, addDoc } from "firebase/firestore";
...
export default function ProjectPage() {
  ...
  const getAllowedNextStatuses = (task: any): TaskStatus[] => {
    ...
    const isAssignedUser = !!user && task.assignedTo === user.uid;

    if (status === "pending") {
      return isAssignedUser ? ["in_progress"] : [];
    }

    if (status === "in_progress") {
      return isAssignedUser ? ["submitted"] : [];
    }

    if (status === "submitted") {
      return isLeader ? ["completed"] : [];
    }
  ...
  const handleExportCSV = () => {
    ...
    const rows = tasks.map(t => [
      esc(t.title),
      esc(getStatusLabel(t.status as TaskStatus)),
      esc(t.category || ""),
  ...
  const completionRate = tasks.length 
    ? Math.round((tasks.filter(t => (t.status as TaskStatus) === 'completed').length / tasks.length) * 100) 
    : 0;

  const completedTasks = tasks.filter((t) => (t.status as TaskStatus) === 'completed');
  const isLateCompletedTask = (task: any) => {
    if ((task.status as TaskStatus) !== 'completed') return false;
    if (!task.submittedAt || !task.dueAt) return false;
    return new Date(task.submittedAt).getTime() > new Date(task.dueAt).getTime();
  };
  const lateCompletedCount = completedTasks.filter(isLateCompletedTask).length;
  const onTimeCompletedCount = completedTasks.length - lateCompletedCount;
  const completedTasksWithTiming = completedTasks
    .filter((task: any) => !!task.submittedAt || !!task.approvedAt)
    .sort((a: any, b: any) => {
      const aTime = new Date(a.approvedAt || a.submittedAt || a.updatedAt || a.createdAt || 0).getTime();
      const bTime = new Date(b.approvedAt || b.submittedAt || b.updatedAt || b.createdAt || 0).getTime();
      return bTime - aTime;
    })
    .slice(0, 6);
  const completedCountForRate = completedTasks.length || 1;
  const onTimeCompletionRate = Math.round((onTimeCompletedCount / completedCountForRate) * 100);
  const lateCompletionRate = Math.round((lateCompletedCount / completedCountForRate) * 100);
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          ...
          <TabsContent value="analytics" className="animate-in fade-in slide-in-from-bottom-2 duration-300">
            ...
            <p className="text-xs text-muted-foreground">
              Late = submittedAt after due date. Approval time is excluded from late/on-time classification.
            </p>
              ...
              <CardContent className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="rounded-lg border p-3 bg-card space-y-2">
                    <p className="text-sm font-semibold">Completion Quality</p>
                    <div className="space-y-1.5 text-xs">
                      <div className="flex items-center justify-between">
                        <span className="text-muted-foreground">On-Time</span>
                        <span className="font-medium">{onTimeCompletedCount} ({onTimeCompletionRate}%)</span>
                      </div>
                      <div className="flex items-center justify-between">
                        <span className="text-muted-foreground">Late</span>
                        <span className="font-medium text-red-500">{lateCompletedCount} ({lateCompletionRate}%)</span>
                      </div>
                    </div>
                  </div>

                  <div className="rounded-lg border p-3 bg-card space-y-2 min-w-0">
                    <p className="text-sm font-semibold">Recent Completed Details</p>
                    {completedTasksWithTiming.length === 0 ? (
                      <p className="text-xs text-muted-foreground">No completed tasks with timing data yet.</p>
                    ) : (
                      <div className="space-y-2">
                        {completedTasksWithTiming.map((task: any) => {
                          const assignedLabel = task.assignedTo
                            ? (getMemberDisplayName(membersByUid.get(task.assignedTo)) || task.assignedToName || "Unassigned")
                            : (task.assignedToName || "Unassigned");
                          const isLate = isLateCompletedTask(task);
                          return (
                            <div key={task.id} className="text-xs border rounded-md p-2 space-y-0.5">
                              <p className="font-medium truncate">{task.title}</p>
                              <p className="text-muted-foreground truncate">Member: {assignedLabel}</p>
                              <p className="text-muted-foreground">
                                Submitted: {task.submittedAt ? format(new Date(task.submittedAt), 'MMM d, yyyy h:mm a') : 'N/A'}
                              </p>
                              <p className="text-muted-foreground">
                                Approved: {task.approvedAt ? format(new Date(task.approvedAt), 'MMM d, yyyy h:mm a') : 'N/A'}
                              </p>
                              {isLate && <p className="text-red-500 font-medium">Late Submission</p>}
                            </div>
                          );
                        })}
                      </div>
