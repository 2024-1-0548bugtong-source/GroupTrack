...
export default function ProjectPage() {
  ...
  // --- Extension-specific: fetch and store members in chrome.storage.local ---
  const [extMembers, setExtMembers] = useState<any[]>([]);
  useEffect(() => {
    if (!isExtension || !projectId) return;
    // Try to load from chrome.storage first
    chrome.storage.local.get([`members_${projectId}`], (result) => {
      if (Array.isArray(result[`members_${projectId}`])) {
        setExtMembers(result[`members_${projectId}`]);
      } else {
        // Fetch from API (absolute URL required)
        fetch(`https://yourdomain.com/api/projects/${projectId}/members`)
          .then((res) => res.json())
          .then((data) => {
            if (Array.isArray(data)) {
              setExtMembers(data);
              chrome.storage.local.set({ [`members_${projectId}`]: data });
            }
          })
          .catch(() => {});
      }
  ...
  // Use extension members if in extension, otherwise normal
  const effectiveMembers = isExtension ? extMembers : members;
  const membersByUid = new Map(effectiveMembers.map((m) => [m.uid, m]));
  ...
  const handleAssigneeChange = async (taskId: string, assigneeUid: string) => {
    ...
    try {
      await updateDoc(doc(db, "projects", projectId, "tasks", taskId), {
        ...
        assignedToName: selectedMember?.displayName || null,
  ...
  const handleExportCSV = () => {
    ...
    const rows = tasks.map(t => [
      ...
      esc(t.category || ""),
        esc(t.assignedTo ? (membersByUid.get(t.assignedTo)?.displayName || t.assignedToName || "") : (t.assignedToName || "")),
  ...
  const latestActivityTs = Math.max(latestTaskActivityTs, new Date(project.updatedAt || project.createdAt || 0).getTime());
                    ...
                    <div className="space-y-3">
                      {columnTasks.map(task => {
                        ...
                        const assignedName = task.assignedTo
                          ? (membersByUid.get(task.assignedTo)?.displayName || task.assignedToName || null)
                          : (task.assignedToName || null);
                        const assignedRole = task.assignedTo
                          ? (membersByUid.get(task.assignedTo)?.role || (task as any).assignedToRole || null)
                          : ((task as any).assignedToRole || null);
                        const assignmentText = assignedName
                          ? `${assignedName}${assignedRole ? ` (${assignedRole})` : ""}`
                          : task.assignedTo
                            ? `Member (${task.assignedTo.slice(0, 6)}...)`
                            : "Unassigned";
                                ...
                                >
                                  <SelectTrigger className="h-7 text-[10px] uppercase font-bold">
                                    <SelectValue
                                      ...
                                      children={(() => {
                                        console.log("[DEBUG] assignedUid:", assignedUid, "members:", members, "membersByUid:", membersByUid);
                                        // Log the actual member object for this UID
                                        const member = membersByUid.get(assignedUid);
                                        console.log("[DEBUG] member for assignedUid:", member, JSON.stringify(member));
                                        if (assignedUid === UNASSIGNED_VALUE) return "Unassigned";
                                        if (member && member.displayName) return `${member.displayName} (${member.role})`;
                                        if (member) return JSON.stringify(member); // Show raw member object for debugging
                                        return "Unassigned";
                                      })()}
                                    />
                                  </SelectTrigger>
