  ...
  const handleTaskTransition = async (task: any, nextStatus: TaskStatus) => {
    ...
    try {
      ...
      if (currentStatus === "in_progress" && nextStatus === "submitted") {
        ...
        try {
          uploadedFiles = await uploadTaskFiles(task.id, "submission", localSubmissionFiles, user.uid);
        } catch {
          // Storage may be unavailable; submission status should still advance.
      ...
      if (nextStatus === "submitted" && attachmentsUnavailable) {
        toast({
