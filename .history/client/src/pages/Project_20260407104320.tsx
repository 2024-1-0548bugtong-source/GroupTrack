...
import { getLocalFileBlob, saveLocalFile } from "@/lib/local-file-storage";
...
function getStatusLabel(status: TaskStatus): string {
  ...
  if (status === "completed") return "Approved (Completed)";
  return "Pending";
}
...
export default function ProjectPage() {
  ...
  const uploadTaskFiles = async (taskId: string, stage: "submission", files: File[], submissionUid: string) => {
    ...
    const uploaded = await Promise.all(
      files.map(async (file) => {
        ...
        const localFileId = `${stage}-${projectId}-${taskId}-${submissionUid}-${fileId}`;
        await saveLocalFile(file, localFileId);
        const storagePath = `local://projects/${projectId}/tasks/${taskId}/submission/${submissionUid}/${fileId}-${file.name}`;
        return {
          fileId,
          storagePath,
          downloadURL: null,
  ...
  const handleOpenTaskFile = async (file: any, download = false) => {
    try {
      let fileUrl: string | null = file.downloadURL || null;
      let shouldRevoke = false;

      if (!fileUrl && file.localFileId) {
        const blob = await getLocalFileBlob(file.localFileId);
        if (!blob) {
          toast({
            title: "File not available on this device",
            description: "This local file was not found in browser storage.",
            variant: "destructive",
          });
          return;
        }
      ...
      link.href = fileUrl;
      link.target = "_blank";
      if (download) link.download = file.name || "task-file";
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);

      if (shouldRevoke) {
        setTimeout(() => URL.revokeObjectURL(fileUrl as string), 2000);
      }
    } catch {
  ...
  const updateTaskStatusOnly = async (taskId: string, updates: Record<string, any>) => {
  ...
  const handleTaskTransition = async (task: any, nextStatus: TaskStatus) => {
    ...
    if (currentStatus === "submitted" && nextStatus === "completed") {
      ...
      updates.officialSubmissionFiles = (task.submissionFiles || []).map((file: any) => ({
        ...file,
        stage: "official",
      }));
    ...
    if (currentStatus === "submitted" && nextStatus === "in_progress") {
      feedback = window.prompt("Request changes note (optional):", "") || "";
    ...
    // 1) Pure Firestore status update path (no upload dependency).
    try {
      console.log("[TaskStatus] Firestore update starting", {
        projectId,
        taskId: task.id,
        fromStatus: currentStatus,
        toStatus: nextStatus,
      ...
      await updateTaskStatusOnly(task.id, updates);
      console.log("[TaskStatus] Firestore update success", {
        projectId,
        taskId: task.id,
        toStatus: nextStatus,
      });
    ...
    // 2) Best-effort event logging (must not block status change).
    try {
      await addDoc(collection(db, "projects", projectId, "tasks", task.id, "events"), {
        ...eventPayload,
        actorUid: user.uid,
      ...
      console.warn("[TaskStatus] Event logging failed (status already updated)", eventError);
    }

    // 3) Best-effort upload after status update for explicit submissions only.
    if (currentStatus === "in_progress" && nextStatus === "submitted") {
      const localSubmissionFiles = submissionFilesByTask[task.id] || [];
      let attachmentsUnavailable = false;

      if (localSubmissionFiles.length > 0) {
        try {
          ...
          console.warn("[TaskStatus] Upload failed after status update", uploadError);
        }
      }
      ...
      if (attachmentsUnavailable) {
        toast({
          ...
          description: "Submission completed, but attachments are unavailable right now.",
        });
      } else {
  ...
  const handleBackfillLegacyTasks = async () => {
    if (!isLeader) return;
    if (!confirm("Backfill legacy tasks with new workflow fields?")) return;
    ...
    try {
      ...
      tasks.forEach((task: any) => {
        ...
        if (task.status !== normalizedStatus) updates.status = normalizedStatus;
        if (!task.updatedAt) updates.updatedAt = task.createdAt || nowIso;

        if (!Array.isArray(task.assignmentFiles)) updates.assignmentFiles = [];
        if (!Array.isArray(task.submissionFiles)) updates.submissionFiles = [];
        if (!Array.isArray(task.officialSubmissionFiles)) updates.officialSubmissionFiles = [];
        if (task.revisionCount === undefined || task.revisionCount === null) updates.revisionCount = 0;

        const syntheticSubmittedAt = task.updatedAt || task.createdAt || nowIso;
        if ((normalizedStatus === "submitted" || normalizedStatus === "completed") && !task.submittedAt) {
          updates.submittedAt = syntheticSubmittedAt;
          updates.submissionDueAt = task.submissionDueAt ?? task.dueAt ?? null;
          if (updates.submissionDueAt) {
            updates.isLateSubmission = new Date(syntheticSubmittedAt).getTime() > new Date(updates.submissionDueAt).getTime();
          } else if (task.isLateSubmission === undefined) {
            updates.isLateSubmission = null;
          }
        }

        if (normalizedStatus === "completed") {
          if (!task.approvedAt) updates.approvedAt = syntheticSubmittedAt;
          if (!task.approvalDecision) updates.approvalDecision = "approved";
          if (Array.isArray(task.submissionFiles) && (!Array.isArray(task.officialSubmissionFiles) || task.officialSubmissionFiles.length === 0)) {
            updates.officialSubmissionFiles = task.submissionFiles.map((file: any) => ({
              ...file,
              stage: "official",
            }));
        ...
        if (Object.keys(updates).length > 0) {
          updatesCount += 1;
          batch.update(doc(db, "projects", projectId, "tasks", task.id), updates);
        }
      });

      if (updatesCount === 0) {
        toast({ title: "No legacy tasks required backfill" });
        return;
      }

      await batch.commit();
      toast({ title: `Backfilled ${updatesCount} task(s)` });
    } catch {
  ...
  const statusData = [
    { name: 'Pending', value: tasks.filter(t => (t.status as TaskStatus) === 'pending').length, color: '#a1a1aa' },
    { name: 'In Progress', value: tasks.filter(t => (t.status as TaskStatus) === 'in_progress').length, color: '#c026a3' },
    { name: 'Submitted for Review', value: tasks.filter(t => (t.status as TaskStatus) === 'submitted').length, color: '#2563eb' },
    { name: 'Approved (Completed)', value: tasks.filter(t => (t.status as TaskStatus) === 'completed').length, color: '#16a34a' },
  ];
  ...
  const completedTasks = tasks.filter((t) => (t.status as TaskStatus) === 'completed');
  ...
  const lateByMemberMap = completedTasks.reduce((acc: Map<string, { name: string; count: number; tasks: string[] }>, task: any) => {
    if (!task.isLateSubmission) return acc;
    // Use submittedByUid if available; fallback to assignedTo in case submission wasn't tracked
    const submitterUid = task.submittedByUid || task.assignedTo;
    if (!submitterUid) return acc;
    const key = submitterUid;
    const label = task.submittedByName || (task.assignedToName) || getMemberDisplayName(membersByUid.get(key));
    let existing = acc.get(key);
    if (!existing) {
      existing = { name: label || "Member", count: 0, tasks: [] };
    }
    existing.count += 1;
    existing.tasks.push(task.title as string);
    acc.set(key, existing);
    return acc;
  }, new Map<string, { name: string; count: number; tasks: string[] }>());
    ...
    <Layout>
      <div className="space-y-8 min-w-0 max-w-full">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          <TabsList className="bg-card border p-1 rounded-xl w-full max-w-full justify-start md:w-auto h-auto grid grid-cols-3 md:flex overflow-x-auto">
          ...
          <TabsContent value="tasks" className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
            <div className="flex flex-wrap justify-between items-center gap-2 min-w-0">
              ...
              <div className="flex gap-2 flex-wrap justify-end min-w-0 max-w-full">
                ...
                {isLeader && (
                  <Button variant="outline" onClick={handleBackfillLegacyTasks}>
                    Backfill Legacy
                  </Button>
                )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
              {['pending', 'in_progress', 'submitted', 'completed'].map((status) => {
                ...
                const columnTasks = tasks.filter(t => (t.status as TaskStatus) === columnStatus);
                return (
                  <div key={status} className="bg-card rounded-xl p-4 border min-h-[400px]">
                    ...
                    <div className="space-y-3">
                      {columnTasks.map(task => {
                        const normalizedStatus = task.status as TaskStatus;
                        ...
                        return (
                          <Card key={task.id} className={`card-interactive status-border-${normalizedStatus} shadow-sm ${
                            ...
                            <CardContent className="p-4">
                              ...
                              <div className="flex items-center justify-between gap-2 mt-3 min-w-0 max-w-full">
                                <Select
                                ...
                                {task.dueAt && (
                                  <span className={`text-xs flex items-center shrink min-w-0 ${
                                  ...
                                  {assignmentFiles.length > 0 && (
                                    <div>
                                      <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-[10px] text-amber-600">Local file (this device only)</span>
                                      </div>
                                      {assignmentFiles.map((f: any) => (
                                        <div key={f.fileId} className="flex items-center gap-2 min-w-0">
                                          <button
                                            onClick={() => handleOpenTaskFile(f, false)}
                                          ...
                                          <button
                                            onClick={() => handleOpenTaskFile(f, true)}
                              ...
                              {(task as any).submittedAt && (normalizedStatus === "submitted" || normalizedStatus === "completed") && (
                                <div className="mt-3 rounded-md border bg-secondary/20 px-2 py-1.5 text-[11px] space-y-1">
                                  ...
                                  {normalizedStatus === "completed" && (task as any).approvedAt && (
                                    <p>
          ...
          <TabsContent value="analytics" className="animate-in fade-in slide-in-from-bottom-2 duration-300">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 mb-8">
              ...
              <Card className="card-interactive">
                <CardHeader className="pb-2">
                  <CardDescription className="text-xs">On-Time Completions</CardDescription>
                  <CardTitle className="text-3xl font-display">{onTimeCompletedCount}</CardTitle>
                </CardHeader>
              </Card>
              <Card className="card-interactive">
                <CardHeader className="pb-2">
                  <CardDescription className="text-xs">Late Completions</CardDescription>
                  <CardTitle className="text-3xl font-display text-red-500">{lateCompletedCount}</CardTitle>
                </CardHeader>
              </Card>
            </div>
            ...
            <Card className="mt-6">
              <CardHeader>
                <CardTitle>Late Tasks Per Member (Approved Only)</CardTitle>
                <CardDescription>Counts only include tasks approved as completed.</CardDescription>
              </CardHeader>
              <CardContent>
                {lateByMember.length === 0 ? (
                  <p className="text-sm text-muted-foreground">No late approved tasks yet.</p>
                ) : (
                  <div className="space-y-3">
                    {lateByMember.map((row) => (
                      <div key={row.name} className="rounded-md border px-3 py-2 space-y-1">
                        <div className="flex items-center justify-between">
                          <span className="text-sm font-medium">{row.name}</span>
                          <Badge variant="secondary">{row.count}</Badge>
                        </div>
                        <div className="text-xs text-muted-foreground space-y-0.5">
                          {row.tasks.map((taskTitle, idx) => (
                            <div key={idx} className="truncate">• {taskTitle}</div>
                          ))}
                        </div>
                      </div>
                    ))}
