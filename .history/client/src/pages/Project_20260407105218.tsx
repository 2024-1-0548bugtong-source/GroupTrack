...
export default function ProjectPage() {
  ...
  const lateByMemberMap = completedTasks.reduce((acc: Map<string, { name: string; count: number; tasks: Array<{ title: string; submittedAt?: string; approvedAt?: string }> }>, task: any) => {
    if (!task.isLateSubmission) return acc;
    
    // Try to identify who submitted: submittedByUid or fallback to task submitter info
    const submitterUid = task.submittedByUid;
    const submitterName = task.submittedByName;
    
    // If we don't have submitter info at all, skip this task
    if (!submitterUid && !submitterName) return acc;
    
    const key = submitterUid || `_unknown_${task.id}`;
    const label = submitterName || getMemberDisplayName(membersByUid.get(submitterUid)) || "Unknown Submitter";
    
    let existing = acc.get(key);
    if (!existing) {
      existing = { name: label, count: 0, tasks: [] };
    }
    existing.count += 1;
    existing.tasks.push({
    ...
    return acc;
  }, new Map<string, { name: string; count: number; tasks: Array<{ title: string; submittedAt?: string; approvedAt?: string }> }>());
      ...
      <div className="space-y-8 min-w-0 max-w-full">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          ...
          <TabsContent value="analytics" className="animate-in fade-in slide-in-from-bottom-2 duration-300">
            ...
            <Card className="mt-6">
              <CardHeader>
                <CardTitle>Late Approvals by Member</CardTitle>
                <CardDescription>{project.name} • Approved tasks submitted after due date</CardDescription>
              </CardHeader>
              <CardContent>
                {lateByMember.length === 0 ? (
                  <p className="text-sm text-muted-foreground">No late approved tasks.</p>
                ) : (
                  <div className="space-y-4">
                    {lateByMember.map((row) => (
                      <div key={row.name} className="rounded-md border px-3 py-2 space-y-2">
                        <div className="flex items-center justify-between">
                        ...
                        <div className="text-xs text-muted-foreground space-y-1">
                          {row.tasks.map((task, idx) => (
                            <div key={idx} className="space-y-0.5">
                              ...
                              {task.submittedAt && (
                                <div className="ml-3 text-[11px] text-amber-600">
                                  Submitted: {format(new Date(task.submittedAt), 'MMM d, yyyy h:mm a')}
                                </div>
                              )}
                              {task.approvedAt && (
                                <div className="ml-3 text-[11px] text-green-600">
                                  Approved: {format(new Date(task.approvedAt), 'MMM d, yyyy h:mm a')}
                                </div>
                              )}
                            </div>
                          ))}
