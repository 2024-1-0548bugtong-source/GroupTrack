...
import { 
  ...
  LayoutDashboard, ListTodo, PieChart, Users, Trash2, UserMinus, Wrench, Loader2 
} from "lucide-react";
import { 
  BarChart, Bar, XAxis, YAxis, Tooltip, ResponsiveContainer, Cell 
} from 'recharts';
import { db } from "@/lib/firebase";
import { collection, doc, updateDoc, deleteDoc, getDoc, setDoc, getDocs, query, where, writeBatch, documentId } from "firebase/firestore";
...
export default function ProjectPage() {
  ...
  const handleRepairMemberNames = async () => {
    if (!isLeader) {
      toast({ title: "Only leaders can run name repair", variant: "destructive" });
      return;
    }

    setRepairingNames(true);
    try {
      const uniqueUids = Array.from(new Set(members.map((m) => m.uid).filter(Boolean)));
      if (uniqueUids.length === 0) {
        toast({ title: "No members to repair" });
        return;
      }
      ...
      const profileByUid = new Map<string, { displayName?: string; email?: string }>();
      ...
      const taskNameByUid = new Map<string, string>();
      tasks.forEach((task) => {
        const uid = task.assignedTo;
        const taskName = typeof task.assignedToName === "string" ? task.assignedToName.trim() : "";
        if (uid && taskName && !isUidLike(taskName) && !taskNameByUid.has(uid)) {
          taskNameByUid.set(uid, taskName);
        }
      });
      ...
      members.forEach((member) => {
        ...
        const profile = profileByUid.get(member.uid);
        const profileName = (profile?.displayName || "").trim();
        const taskName = (taskNameByUid.get(member.uid) || "").trim();
        const emailPrefix = profile?.email?.includes("@") ? profile.email.split("@")[0].trim() : "";

        let resolved = "";
        if (current && !isUidLike(current)) resolved = current;
        else if (profileName && !isUidLike(profileName)) resolved = profileName;
        else if (taskName && !isUidLike(taskName)) resolved = taskName;
        else if (emailPrefix && !isUidLike(emailPrefix)) resolved = emailPrefix;
        else if (member.uid === user?.uid) {
          const selfFallback = (user?.displayName || user?.email?.split("@")[0] || "").trim();
          if (selfFallback && !isUidLike(selfFallback)) resolved = selfFallback;
        }

        if (!resolved) {
          resolved = `Member-${member.uid.slice(0, 4)}`;
        }

        resolvedNameByUid.set(member.uid, resolved);
      });
      ...
      members.forEach((member) => {
        const nextName = resolvedNameByUid.get(member.uid) || "";
        const currentName = (member.displayName || "").trim();
        if (nextName && nextName !== currentName) {
          batch.update(doc(db, "projects", projectId, "members", member.id), {
      ...
      tasks.forEach((task) => {
        ...
        const nextName = resolvedNameByUid.get(task.assignedTo);
        if (!nextName) return;

        const currentTaskName = typeof task.assignedToName === "string" ? task.assignedToName.trim() : "";
        if (!currentTaskName || isUidLike(currentTaskName) || currentTaskName !== nextName) {
          batch.update(doc(db, "projects", projectId, "tasks", task.id), {
            assignedToName: nextName,
            updatedAt: new Date().toISOString(),
          });
          taskUpdates += 1;
        }
      ...
      if (memberUpdates === 0 && taskUpdates === 0) {
        toast({ title: "No UID-like names found" });
        return;
      }

      await batch.commit();
      toast({
        title: "Name repair completed",
        description: `Updated ${memberUpdates} member profiles and ${taskUpdates} task assignee labels.`,
      });
    } catch (e) {
      toast({ title: "Name repair failed", variant: "destructive" });
    } finally {
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="flex gap-2">
                ...
                {isLeader && (
                  <Button
                    variant="outline"
                    onClick={handleRepairMemberNames}
                    disabled={repairingNames || members.length === 0}
                  >
                    {repairingNames ? (
                                  ...
                                  <SelectTrigger className="h-7 text-[10px] uppercase font-bold">
                                    <SelectValue
                                      ...
                                      children={(() => {
                                        ...
                                        if (member) {
                                          const taskAssignedName = typeof task.assignedToName === "string" ? task.assignedToName.trim() : "";
                                          const preferredName = taskAssignedName && !isUidLike(taskAssignedName)
                                            ? taskAssignedName
                                            : getMemberDisplayName(member);
                                          return `${preferredName}${member.role ? ` (${member.role})` : ""}`;
                                        }
