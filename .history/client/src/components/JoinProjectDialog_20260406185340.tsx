...
export function JoinProjectDialog() {
  ...
  const handleJoin = async () => {
    ...
    try {
      ...
      // Check if user is already a member (prevents duplicates on re-login)
      const memberRef = doc(db, "projects", projectId, "members", user.uid);
      const memberSnap = await getDoc(memberRef);
      
      if (memberSnap.exists()) {
        // Already a member — just navigate to the project
        toast({
          title: "Welcome back!",
          description: `You're already a member of ${projectDoc.data().name}`,
        });
        setOpen(false);
        setCode("");
      ...
      // Add user to members
      const memberName = (nickname || user.displayName || user.email?.split("@")[0] || "Member").trim();
