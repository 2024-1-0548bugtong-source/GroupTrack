...
// Copy index.html from dist/public to dist-extension
const publicDir = path.join(root, "dist", "public");
const extAssetsDir = path.join(dist, "assets");
...
// Copy all assets from dist/public/assets to dist-extension/assets
...
if (fs.existsSync(srcAssetsDir)) {
  for (const file of fs.readdirSync(srcAssetsDir)) {
    fs.copyFileSync(
      ...
      path.join(extAssetsDir, file)
    );
    console.log(`✓ Copied asset: ${file}`);
  }
} else {
  console.warn("⚠ No assets found in dist/public/assets");
}
