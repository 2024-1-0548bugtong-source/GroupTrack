...
Quick extension safeguard (prevents 404 in unpacked mode)
When testing in chrome://extensions, always load the unpacked extension from dist-extension after running npm run build:extension.
Do not load the extension/ source folder directly, because it does not contain the built popup assets and can show:
404 Page Not Found
Did you forget to add the page to the router?

If you already loaded the wrong folder:
1) Remove the unpacked extension
2) Rebuild with npm run build:extension
3) Load unpacked from dist-extension
4) Click Reload after each rebuild during testing
